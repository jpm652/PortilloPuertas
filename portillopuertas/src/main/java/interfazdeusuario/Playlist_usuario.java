...
//	private event _compartir_playlist;
	...
	public Playlist_usuario(Playlist playlist,UsuarioComun usuario) {
		inicializar(new VerticalLayout(),playlist,usuario);
	}
	...
	private void inicializar(VerticalLayout vlpadre,Playlist playlist,UsuarioComun usuario) {
		...
		this.setNombreautor(playlist.getUsuarioCreador());
					...
					public void onComponentEvent(ClickEvent<Button> event) {
						...
						if (campoNombre.isEmpty()) {
							...
							if (casos == 0) {
								...
								Dialog dialog = new Dialog();
								VerticalLayout dialogLayout = createDialogLayout(dialog, "Cancion añadida correctamente",
	...
	public void CargarCancionesPlaylist(VerticalLayout vl,Playlist playlist,UsuarioComun usuario) {
		...
		for (int i = 0; i < cancionesplaylist.length; i++) {
			cancion = new ListaCanciones(vl, cancionesplaylist[i],usuario);
